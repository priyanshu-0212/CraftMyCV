...
class DashboardManager:
    ...
    def render_admin_panel(self):
        ...
        # Add a danger zone section with clear database button
        st.sidebar.markdown("---")
        st.sidebar.markdown("### ⚠️ Danger Zone")
        
        # Add a confirmation checkbox
        confirm_clear = st.sidebar.checkbox("I understand this will delete ALL resume data", key="confirm_clear")
        
        if st.sidebar.button("🗑️ Clear Database", disabled=not confirm_clear, type="primary"):
            result = self.clear_resume_data()
            if result["success"]:
                ...
                try:
                    ...
                    cursor.execute(
                        ...
                        (st.session_state.get('current_admin_email', 'admin'), "cleared_database")
                    )
                    self.conn.commit()
                except Exception as e:
                    st.sidebar.warning(f"Action logged with error: {e}")
            else:
    ...
    def render_resume_data_section(self):
        ...
        if resume_data:
            ...
            with st.container():
                ...
                # Add download buttons and delete button
                ...
                with col3:
                    ...
                    delete_clicked = st.button("🗑️ Delete Filtered Data", type="primary", key="delete_filtered_data")
                    
                    # Create a confirmation mechanism when delete is clicked
                    if delete_clicked:
                        st.warning(f"Are you sure you want to delete {len(filtered_df)} filtered resume records?")
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("✅ Yes, Delete", key="confirm_delete"):
                                ...
                                result = self.delete_filtered_data(filter_conditions)
                                if result["success"]:
                                    ...
                                    try:
                                        ...
                                        cursor.execute(
                                            "INSERT INTO admin_logs (admin_email, action) VALUES (?, ?)",
                                            (st.session_state.get('current_admin_email', 'admin'), f"deleted_{result['count']}_filtered_records")
                                        )
                                        self.conn.commit()
                                    except Exception as e:
                                        st.warning(f"Action logged with error: {e}")
                                    # Refresh the page
    ...
    def clear_resume_data(self):
        """Clear all resume data from the database"""
        cursor = self.conn.cursor()
        try:
            # Start a transaction
            cursor.execute("BEGIN TRANSACTION")
            
            # Delete from resume_analysis first (because of foreign key constraints)
            ...
            return {"success": True, "message": "All resume data successfully deleted"}
        except Exception as e:
            # Rollback in case of error
            cursor.execute("ROLLBACK")
            return {"success": False, "message": f"Error deleting resume data: {str(e)}"}

    def delete_filtered_data(self, filter_conditions):
        """Delete filtered resume data from the database"""
        cursor = self.conn.cursor()
        try:
            # Start a transaction
            cursor.execute("BEGIN TRANSACTION")
            
            # Build the WHERE clause for the filters
            where_clauses = []
            params = []
            
            if filter_conditions.get("role") and filter_conditions["role"] != "All":
                where_clauses.append("target_role = ?")
                params.append(filter_conditions["role"])
                
            if filter_conditions.get("category") and filter_conditions["category"] != "All":
                where_clauses.append("target_category = ?")
                params.append(filter_conditions["category"])
                
            where_clause = " AND ".join(where_clauses) if where_clauses else "1=1"
            
            # Get IDs of resume data to delete
            cursor.execute(f"SELECT id FROM resume_data WHERE {where_clause}", params)
            resume_ids = [row[0] for row in cursor.fetchall()]
            
            if not resume_ids:
                return {"success": False, "message": "No records match the filter criteria", "count": 0}
            
            # Delete from resume_analysis first (foreign key constraint)
            placeholders = ", ".join(["?"] * len(resume_ids))
            cursor.execute(f"DELETE FROM resume_analysis WHERE resume_id IN ({placeholders})", resume_ids)
            
            # Delete from resume_data
            cursor.execute(f"DELETE FROM resume_data WHERE id IN ({placeholders})", resume_ids)
            
            # Commit the transaction
            self.conn.commit()
            
            return {"success": True, "message": f"Successfully deleted {len(resume_ids)} records", "count": len(resume_ids)}
            
        except Exception as e:
            # Rollback in case of error
            self.conn.rollback()
            return {"success": False, "message": f"Error deleting records: {str(e)}", "count": 0}
